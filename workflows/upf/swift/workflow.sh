...
if [ ${site2} == "summit" -a "x${CANDLE_RUN_WORKFLOW:--1}" != "x1" ] # ALW 2020-11-15: If we're running the candle wrapper scripts in which case if this file were being called then $CANDLE_RUN_WORKFLOW=1, don't set $TURBINE_LAUNCH_OPTIONS as this variable and the settings in the declaration below are handled by the wrapper scripts
then
